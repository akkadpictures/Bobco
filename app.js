...
function totalMins(){
  return [...state.services].reduce((t,id) => {
    const s = svcById(id);
    if(!s) return t;
    const d = Number(s.duration_min);
    return t + (Number.isFinite(d) ? d : 30);
  }, 0);
}
...
function syncTicket(){
  ...
  if(!items.length){
    ...
    body.innerHTML = items.map(s => {
        const priceTxt = NO_PRICE.includes(s.name) ? '<span class="p later">الحساب لاحقاً</span>' : `<span class="p">${fmtSYP(s.price)}</span>`;
        return `<div class="tline"><span class="n">${s.name}</span>${priceTxt}</div>`;
      }).join('')
